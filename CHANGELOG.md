...
手動 JSON 分頁定位為開發者模式：自動承接查詢分頁的 request，並可整包交給 IT。

### Proxy 回報實際送出的 request

- guided 模式的回應新增 `request` 欄位，內含**真正送往上游的 URL、body 與 query parameters**。
- 先前前端只知道「表單填了什麼」，不知道 proxy 做了哪些決定 —— 是否補上 `HwkeyDownloadStatus=all`、是否因為值為空而省略 `SITE`。要當除錯工具，這個落差必須補掉。
- **查詢失敗時也會回傳 `request`**，因為出問題的那一次正是最需要檢視的。

### 自動帶入

- 從查詢分頁切換到手動分頁時，自動填入最後一次送出的 request（body 與 parameters）。
- 若手動分頁的 body 已被手動編輯過，**不會覆蓋** —— 改在標題列顯示「查詢分頁有較新的 request」與「帶入」按鈕，由使用者決定。

### 交給 IT 的匯出功能

- **複製 cURL**：產生可直接執行的指令，含正確的 shell 跳脫（單引號以 `'\''` 處理）。實測產生的指令可原樣貼進終端機執行成功。
- **複製完整記錄**：Markdown 格式，含時間、環境、完整 request（URL + body）與 response（狀態碼、耗時、位元組數、內容），可直接貼進郵件或 Teams。回應超過 20,000 字元會截斷並註明，避免憑證內容把訊息撐爆。
