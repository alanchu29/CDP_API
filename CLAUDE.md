...
介面有五種主題，切換器在標題列。**`midnight` 是 `:root` 的裸色盤，不帶 `data-theme` 屬性**；其餘四個（`terminal`、`daylight`、`industrial`、`slate`）以 `[data-theme="..."]` 覆寫整組變數。選擇記在 localStorage。

規則：**`:root` 以外的地方不得硬編碼任何顏色。** 漏一個，該處在其他主題就會沿用 midnight 的深色值 —— 在 `daylight` 上就是深底深字，整塊看不見。

新增主題或變數時，把暫存區的兩支檢查跑過：

- `check-themes.mjs` —— 確認每個主題都覆寫了 `:root` 的全部 32 個必要變數，並抓出拼錯的變數名
- `contrast.mjs` —— 計算 17 組前景／背景的 WCAG 對比，全部必須達標

這兩支不在 repo 裡（是一次性工具），但邏輯很短，需要時重寫即可。沒有它們就只能靠肉眼，而肉眼看不出 2.97:1 和 3.0:1 的差別。
