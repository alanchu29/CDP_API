...
- **查詢**：日常使用。proxy 決定 query parameters、省略空值 key、把回應正規化成 `{ records }`。
- **手動 JSON**：開發者模式。送出手寫的 body 與自訂 query parameters，回應完全不正規化 —— 狀態碼、耗時、位元組數、原始 body 都如實呈現。上游的錯誤訊息只有在這裡看得到（查詢分頁會包成 502）。

guided 模式的回應帶有 `request` 欄位，記錄**實際送往上游的內容**。切換到手動分頁時會自動帶入，也是「複製 cURL」與「複製完整記錄」的來源。改動 proxy 建構 request 的邏輯時，記得 `request` 必須跟著更新，否則除錯資訊會與實際行為不符 —— 那比沒有更糟。

**raw 模式的 endpoint 與 method 由伺服器端寫死**（`POST /sn_info`）。規格中另外三支 `PUT` API 會寫入正式資料庫，而部署後的頁面沒有存取控制，所以瀏覽器不能碰得到它們。要開放必須先有驗證機制。

## 介面風格

五種風格，切換器在標題列，選擇記在 localStorage。**`midnight` 是 `:root` 的裸樣式，不帶 `data-theme` 屬性**；其餘四個以 `[data-theme="..."]` 覆寫。

| 風格 | 處理方式 |
|---|---|
| midnight | 深色框線卡片 |
| flat | 無框線、純色塊、斑馬紋表格 |
| elevated | 無框線、靠陰影分層、大圓角 |
| outline | 透明填色、細線稿、底線式輸入框、幽靈按鈕 |
| paper | 淺色暖白、襯線標題、細線分隔 |

`:root` 的變數分兩段：

- **色盤**（32 個字面色值）—— 每個風格都**必須逐一覆寫**。漏一個就會沿用 midnight 的深色值，在 `paper` 上就是深底深字，整塊看不見。
- **處理方式**（treatment）—— 卡片框線/陰影/內距、輸入框填色與框線、按鈕實心或幽靈、表格列是細線或斑馬紋、標籤排版、標題字體。這些多半引用色盤變數（如 `--panel-bg: var(--surface)`），所以**沿用預設是正確的**，不需要每個風格都寫。

**風格差異只做視覺處理，不動版面。** 元素位置、層級、資訊架構在五種風格下完全相同 —— 這是刻意的，改版面是另一件事。

`:root` 以外不得硬編碼任何顏色。新增風格或變數後，用暫存區的兩支檢查驗證（邏輯很短，需要時重寫）：

- `check-themes.mjs` —— 每個風格是否覆寫全部色盤變數，並抓出拼錯的變數名（曾抓到 `--warn-text: #efc committee` 這種手滑）
- `contrast.mjs` —— 五個風格 × 17 組前景／背景的 WCAG 對比，全部必須達標

肉眼看不出 3.99:1 和 4.5:1 的差別，這種事只能算。
